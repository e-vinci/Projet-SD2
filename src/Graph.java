...
class Graph {
  ...
  public void calculerItineraireMinimisantNombreRoutes(String depart, String arrivee) {
    ...
    System.out.println("Trajet de " + depart + " à " + arrivee + ": " + (path.size()-1) + " routes et " + visited.get(destination) + " km");
  ...
  public void calculerItineraireMinimisantKm(String depart, String arrivee) {
    ...
    // Print the path
    System.out.println("Itinéraire le plus court en kilomètre");
    System.out.println("Pour aller de " + depart + " à " + arrivee + ", il est possible de trouver un itinéraire de " + finals.get(end) + " km.");
    System.out.println();
    System.out.println("Trajet de " + depart + " à " + arrivee + ": " + (path.size() - 1) + " routes et " + finals.get(end) + " kms");
    for (int i = 0; i < path.size() - 1; i++) {
      City city1 = path.get(i);
      City city2 = path.get(i + 1);
      double distance = Util.distance(city1.getLongitude(), city1.getLatitude(), city2.getLongitude(), city2.getLatitude());
      System.out.println(city1.getName() + " -> " + city2.getName() + " (" + distance + " km)");
    }
  }
...
class Graph {
  ...
  public Graph(File city, File road) {
    ...
    try (Scanner cityScanner = new Scanner(city)) {
      while (cityScanner.hasNextLine()) {
        ...
        cities.add(newCity);
        listeDadjacences.put(newCity,
            new HashSet<>()); // Initialiser un ensemble vide pour chaque ville
      }
    } catch (FileNotFoundException e) {
      e.printStackTrace();
    }
    ...
    try (Scanner roadScanner = new Scanner(road)) {
      while (roadScanner.hasNextLine()) {
        ...
        roads.add(newRoad);
        // Mettre à jour la map listeDadjacences avec les routes connectées à chaque ville
        City city1 = findCityById(cityId1);
        City city2 = findCityById(cityId2);
        listeDadjacences.get(city1).add(newRoad);
        listeDadjacences.get(city2).add(newRoad);
      }
    } catch (FileNotFoundException e) {
      e.printStackTrace();
    }
  ...
  public void calculerItineraireMinimisantNombreRoutes(String depart, String arrivee) {
    ...
    HashMap<City, City> previousCity = new HashMap<>();
    file.offer(source);
    visited.put(source, 0);

    while (!file.isEmpty()) {
      City current = file.poll();

      for (Road road : listeDadjacences.get(current)) {
        City neighbor =
            (road.getIdCitySource() == current.getId()) ? findCityById(road.getIdCityDestination())
                : findCityById(road.getIdCitySource());
        if (!visited.containsKey(neighbor)) {
          visited.put(neighbor, visited.get(current) + 1);
          previousCity.put(neighbor, current);
          file.offer(neighbor);
  ...
  public void calculerItineraireMinimisantKm(String depart, String arrivee) {
    ...
    pq.offer(new CityDistance(source, 0.0));
